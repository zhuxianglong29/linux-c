...
static hash_t *s_ip_count_hash;//存储IP与连接数
static hash_t *s_pid_ip_hash;//pid与ip对应哈希表
...
int main(void)
{
	...
	//test str_upper     	
	//char str ="abcDef";  // 指针指向一个字符串常量，常量不能被修改
	char str3[] = "abcDef";
	str_upper(str3);
	printf("str3=%s\n", str3);
	...
	*/
	...
	daemon(0, 0);//daemon(1, 1);变车守护进程后台工作00表示无打印信息，第一个表示不更改目录，所以放在parseconf后面
	//配置测试
...
*/
	...
	signal(SIGCHLD,handle_sigchld);//忽略信号，解决僵尸进程
