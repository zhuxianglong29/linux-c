...
{
	int sock;
	if((sock = socket(PF_INET,SOCK_STREAM,0))<0)//socket()函数(IPV4,流式套接字，协议类型：0自动选择）
		ERR_EXIT("tcp_client");
...
{
	int listenfd;
	if((listenfd = socket(PF_INET,SOCK_STREAM,0))<0)//socket()函数(IPV4,流式套接字，协议类型：0自动选择）
		ERR_EXIT("tcp_server");
	...
	if(host!=NULL)
	{
		...
		servaddr.sin_addr.s_addr =htonl(INADDR_ANY);//sin_addr表示32位的IP地址，INADDR_ANY就是inet_addr("0.0.0.0")服务器表监听所有网卡IP
