...
int list_common(session_t *sess,int detail)//获取文件列表信息相当ls -l
{
		DIR *dir = opendir(".");
	if (dir == NULL)
	{
...
{	
	...
	elapsed += (double)(curr_usec - sess->bw_transfer_start_sec) / (double)1000000;
	if (elapsed <= (double)0)
	{
		elapsed = (double)0.01;
	}
...
static void do_retr(session_t *sess)
{
	...
	*/
	...
	// ssize_t sendfile(int out_fd, int in_fd, off_t *offset, size_t count);直接内核发送，效率较高
...
static void do_stor(session_t *sess)//上传和断点续传
{
	
	upload_common(sess,0);
}
static void do_appe(session_t *sess)//appe断点续传
{
...
static void do_rnfr(session_t *sess)//保存文件名信息
{
...
static void do_rnto(session_t *sess)//必须先调用do_rnfr重命名
{
...
static void do_site(session_t *sess)//更改权限
{
