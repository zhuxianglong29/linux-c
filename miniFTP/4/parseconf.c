...
void parseconf_load_file(const char *path)
{
	...
	while (fgets(setting_line,sizeof(setting_line),fd)!=NULL)//fgets读一行
	{
		if(strlen(setting_line)==0||setting_line[0]=='#'||str_all_space(setting_line))
			continue;//进入下一循环，读下一行
...
void parseconf_load_setting(const char *setting)
{
	...
	
	{
		...
		{
			if(strcmp(key, p_str_setting->p_setting_name)==0)//判断是否相同
			{
				const char **p_cur_setting=p_str_setting->p_variable;
				if(*p_cur_setting)
					 free((char*)*p_cur_setting);//先清空已有数据	
				
				*p_cur_setting=strdup(value);//使用strdup以申请内存的方式赋值防止产生野指针
				return;
	...
	
	{
		...
		{
			if(strcmp(key, p_bool_setting->p_setting_name)==0)//判断是否相同
			{
				str_upper(value);
	...
	
	{
		...
		while (p_uint_setting->p_setting_name != NULL)
		{
			...
			{
				if (value[0] == '0')
					*(p_uint_setting->p_variable) = str_octal_to_uint(value);//8进制转整形
				else
					*(p_uint_setting->p_variable) = atoi(value);//字符串数字转整形
